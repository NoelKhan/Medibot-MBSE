...
class PharmacyService {
  ...
  async getNearbyPharmacies(userLocation?: { latitude: number; longitude: number }): Promise<Pharmacy[]> {
  ...
  async getMedicationHistory(userId: string): Promise<{ medication: Medication; timestamp: string }[]> {
    try {
      const key = `@medibot_medication_history_${userId}`;
