...
export class RoleGuard {
  ...
  static requirePatient(user: User | null, navigation: any): boolean {
    ...
    if (this.isStaff(user)) {
      Alert.alert(
        'Access Denied',
        'Staff members cannot access patient features. Please login as a patient or use the staff portal.',
        [
          {
  ...
  static requireStaff(user: User | null, navigation: any): boolean {
    if (!user) {
      Alert.alert(
        'Authentication Required',
        'Please login to access staff features.',
        [
          {
            text: 'Login',
            onPress: () => navigation.replace('RoleSelection')
    ...
    if (!this.isStaff(user)) {
      Alert.alert(
        'Access Denied',
        'This feature is only available to staff members. Please login with a staff account.',
        [
          {
            text: 'Go to Role Selection',
            onPress: async () => {
