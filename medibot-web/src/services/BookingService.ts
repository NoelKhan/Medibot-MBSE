...
class BookingService implements BookingAPI {
  ...
  constructor() {
    ...
    this.config = {
      baseURL: import.meta.env.VITE_BOOKING_API_URL || 'https://api.healthbooking.com.au',
      apiKey: import.meta.env.VITE_BOOKING_API_KEY || 'demo-api-key',
      endpoints: {
  ...
  async searchDoctors(specialty?: string, location?: string, date?: Date): Promise<Doctor[]> {
    try {
      ...
      const hasBackendConfig = import.meta.env.VITE_BOOKING_API_URL && 
                               import.meta.env.VITE_BOOKING_API_URL !== 'https://api.healthbooking.com.au';
  ...
  private async getCurrentUserId(): Promise<string> {
    try {
      const userStr = await WebStorage.getItem('currentUser');
      if (userStr) {
  ...
  async cancelAppointment(appointmentId: string, reason: string): Promise<boolean> {
    try {
      ...
      await WebStorage.setItem('@medibot_appointments', JSON.stringify(updatedAppointments));
      return true;
    ...
    try {
      ...
      await WebStorage.setItem('@medibot_appointments', JSON.stringify(formattedAppointments));
  ...
  private async saveAppointment(appointment: Appointment): Promise<void> {
    try {
      ...
      await WebStorage.setItem('@medibot_appointments', JSON.stringify(appointments));
    } catch (error) {
  ...
  private async getStoredAppointments(): Promise<Appointment[]> {
    try {
      const stored = await WebStorage.getItem('@medibot_appointments');
      if (!stored) return [];
