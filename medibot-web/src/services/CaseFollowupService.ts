...
class CaseFollowupService {
  ...
  constructor() {
    // Notification service will be initialized on demand
    try {
      this.pushNotificationService = PushNotificationService.getInstance();
    } catch (error) {
      console.warn('PushNotificationService not available:', error);
    }
  }
  ...
  async processReminders(): Promise<void> {
    ...
    for (const followup of pendingFollowups) {
      const _hoursSinceScheduled = (now.getTime() - followup.scheduledDate.getTime()) / (1000 * 60 * 60);
  ...
  private async scheduleFollowupNotification(followup: CaseFollowup): Promise<void> {
    ...
    if (followup.userEmail && (followup.userId === 'guest' || followup.userId.startsWith('guest_'))) {
      ...
      // For registered users, use push notifications if available
      try {
  ...
  private async sendFollowupReminder(followup: CaseFollowup): Promise<void> {
    ...
    if (followup.userEmail && (followup.userId === 'guest' || followup.userId.startsWith('guest_'))) {
      ...
      // Notifications not fully implemented for web yet
      console.log('Would send notification:', { title, followupId: followup.id });
    }
  ...
  private async escalateCase(followup: CaseFollowup, daysOverdue: number): Promise<void> {
    ...
    // Send escalation notification to healthcare team (not implemented for web yet)
    logger.warn(escalationMessage, { followupId: followup.id, caseId: followup.caseId });
  ...
  private async queueEmailNotification(followup: CaseFollowup): Promise<void> {
    ...
    await WebStorage.setItem(this.EMAIL_QUEUE_KEY, JSON.stringify(emailQueue));
  }
...
`;
  ...
  private async analyzeFollowupResponse(response: FollowupResponse): Promise<void> {
    ...
    if (!response.feelingBetter || response.requiresFurtherCare || response.newSymptoms.length > 0) {
      ...
      if (followup) {
        ...
        // Notifications not fully implemented for web yet
        console.log('Patient response indicates ongoing concerns. Review required', { responseId: response.followupId });
      }
    }
  }
  ...
  private async saveFollowup(followup: CaseFollowup): Promise<void> {
    ...
    await WebStorage.setItem(this.STORAGE_KEY, JSON.stringify(followups));
  }
  ...
  private async saveFollowupResponse(response: FollowupResponse): Promise<void> {
    try {
      const stored = await WebStorage.getItem(this.RESPONSES_KEY);
      const responses = stored ? JSON.parse(stored) : [];
      responses.push(response);
      await WebStorage.setItem(this.RESPONSES_KEY, JSON.stringify(responses));
    } catch (error) {
  ...
  private async getEmailQueue(): Promise<any[]> {
    try {
      const stored = await WebStorage.getItem(this.EMAIL_QUEUE_KEY);
