...
export class AuthService {
  ...
  public async refreshAccessToken(): Promise<void> {
    try {
      const refreshToken = await WebStorage.getItem(STORAGE_KEYS.REFRESH_TOKEN);
      ...
      await WebStorage.setItem(STORAGE_KEYS.AUTH_TOKEN, response.accessToken);
      if (response.refreshToken) {
        await WebStorage.setItem(STORAGE_KEYS.REFRESH_TOKEN, response.refreshToken);
      }
  ...
  public async loadAuthState(): Promise<AuthState | null> {
    try {
      const userDataStr = await WebStorage.getItem(STORAGE_KEYS.USER_DATA);
      const accessToken = await WebStorage.getItem(STORAGE_KEYS.AUTH_TOKEN);
      const refreshToken = await WebStorage.getItem(STORAGE_KEYS.REFRESH_TOKEN);
      const timestampStr = await WebStorage.getItem(STORAGE_KEYS.AUTH_TIMESTAMP);
  ...
  public async updateUserData(user: User | PatientUser): Promise<void> {
    try {
      this.currentUser = user;
      await WebStorage.setItem(STORAGE_KEYS.USER, JSON.stringify(user));
      await WebStorage.setItem(STORAGE_KEYS.USER_DATA, JSON.stringify(user));
  ...
  private async saveAuthState(
    ...
    try {
      ...
      await WebStorage.setItem(STORAGE_KEYS.USER_DATA, JSON.stringify(user));
      await WebStorage.setItem(STORAGE_KEYS.AUTH_TOKEN, accessToken);
      await WebStorage.setItem(STORAGE_KEYS.REFRESH_TOKEN, refreshToken);
      await WebStorage.setItem(STORAGE_KEYS.AUTH_TIMESTAMP, timestamp.toString());
  ...
  private async clearAuthState(): Promise<void> {
    try {
      ...
      await WebStorage.removeItem(STORAGE_KEYS.REFRESH_TOKEN);
      await WebStorage.removeItem(STORAGE_KEYS.USER_DATA);
      await WebStorage.removeItem(STORAGE_KEYS.AUTH_TIMESTAMP);
      
      // Clear regular storage
      await WebStorage.removeItem(STORAGE_KEYS.USER);
      await WebStorage.removeItem(STORAGE_KEYS.GUEST_USER);
  ...
  private async saveGuestToStorage(user: User | PatientUser): Promise<void> {
    try {
      await WebStorage.setItem(STORAGE_KEYS.GUEST_USER, JSON.stringify(user));
    } catch (error) {
  ...
  private async loadGuestFromStorage(): Promise<PatientUser | null> {
    try {
      const guestData = await WebStorage.getItem(STORAGE_KEYS.GUEST_USER);
      if (guestData) {
